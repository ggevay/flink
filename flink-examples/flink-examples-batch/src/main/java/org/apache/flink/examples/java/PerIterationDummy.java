...
public class PerIterationDummy {
...
//		DataSet<Integer> next = iteration.flatMap(new FlatMapFunction<Integer, Integer>() {
//			@Override
//			public void flatMap(Integer x, Collector<Integer> out) throws Exception {
//				out.collect(x - 1);
//			}
//		});

		DataSet<Integer> next = iteration.flatMapSuperstepNumber(new FlatMapFunction<Integer, Integer>() {
			@Override
			public void flatMap(Integer stepNum, Collector<Integer> out) throws Exception {
				out.collect(stepNum);
			}
		}, Integer.class).setParallelism(1).rebalance().setParallelism(1);

//		DataSet<Integer> xxx = next.flatMap(new FlatMapFunction<Integer, Integer>() {
//			@Override
...
//		}).setParallelism(1);
		...
		DataSet<Integer> xxx = next.map(new MapFunction<Integer, Integer>() {
			@Override
		...
		DataSet<Integer> yyy = xxx.map(new MapFunction<Integer, Integer>() {
			@Override
			public Integer map(Integer value) throws Exception {
				return value;
			}
		...
		//DataSet<Integer> result = iteration.closeWith(xxx, xxx);
		DataSet<Integer> result = iteration.closeWith(yyy, yyy);
