 ...
 */
public class OutputCollector_1writer<T> implements Collector<T> {

	// list of writers
	private final RecordWriter<SerializationDelegate<T>> writer;
	...
	public OutputCollector_1writer(List<RecordWriter<SerializationDelegate<T>>> writers, TypeSerializer<T> serializer) {
		this.delegate = new SerializationDelegate<T>(serializer);
		this.writer = ((RecordWriter<SerializationDelegate<T>>[]) writers.toArray(new RecordWriter[writers.size()]))[0];
	}
	...
	public List<RecordWriter<SerializationDelegate<T>>> getWriters() {
		return Collections.singletonList(writer);
	}
}
