...
public class DatalogIterationNode extends TwoInputNode implements IterationNode {
	...
	public void setDatalogMerge(OptimizerNode datalogMerge, ExecutionMode executionMode) {

		// there needs to be at least one node in the workset path, so
		// if the next workset is equal to the workset, we need to inject a no-op node
		if (datalogMerge == worksetNode || datalogMerge instanceof BinaryUnionNode) {
			NoOpNode noop = new NoOpNode();
			noop.setParallelism(getParallelism());

			DagConnection noOpConn = new DagConnection(datalogMerge, noop, executionMode);
			noop.setIncomingConnection(noOpConn);
			datalogMerge.addOutgoingConnection(noOpConn);
		...
		this.datalogMergeRootConnection = new DagConnection(datalogMerge, executionMode);
	}
	...
	public String getOperatorName() {
		if (datalogMerge == null) {
			return "Workset Iteration";
		} else {
			return "Workset Iteration (Datalog)";
		}
	}
